...
import com.eternalnovices.cotasker.service.bussineslogic.concrete.tarea.ConsultarTareaUseCase;
import com.eternalnovices.cotasker.service.bussineslogic.validator.concrete.tarea.ConsultarTareaValidator;
...
public class ConsultarTareaFacade implements FacadeFind<TareaDTO>{

	@Override
	public List<TareaDTO> execute(final TareaDTO dto) {
		final TareaDomain domain = TareaDTOMapper.convertToDomain(dto);
		ConsultarTareaValidator.ejecutar(domain);
		List<TareaDTO> resultados = new ArrayList<TareaDTO>();
		
		DAOFactory daofactory = DAOFactory.obtenerDAOFactory(TipoDAOFactory.SQLSERVER);
		
		try {
			daofactory.iniciarTransaccion();
			var useCase = new ConsultarTareaUseCase(daofactory);
			resultados = TareaDTOMapper.convertToListDTO(useCase.execute(domain));
			...
			daofactory.cancelarTransaccion();
			final var mensajeUsuario = CatalogoMensajes.obtenerContenidoMensaje(CodigoMensaje.M0000000807);
			final var mensajeTecnico = CatalogoMensajes.obtenerContenidoMensaje(CodigoMensaje.M0000000808);
